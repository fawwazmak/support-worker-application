...
const YoungPersons = () => {
  ...
  const { selectedYpCandidate } = state || {};
  const [currentTab, setCurrentTab] = useState("service");


  const [showAllRiskAssessment, setShowAllRiskAssessment] = useState(false);
  const riskAssementTodisplay = showAllRiskAssessment ? selectedYpCandidate.riskAssesment || [] : selectedYpCandidate.riskAssesment.slice(0, 5) || [];
  const [showAllCaseNote, setShowAllCaseNote] = useState(false);
  const caseNoteTodisplay = showAllCaseNote ? selectedYpCandidate.casenote || [] : selectedYpCandidate.casenote.slice(0, 5) || [];
  const [showAllComplaints, setShowAllComplaints] = useState(false);
  const complaintsTodisplay = showAllComplaints ? selectedYpCandidate.complaints || [] : selectedYpCandidate.complaints.slice(0, 5) || [];
  ...
  return (
    <div className='h-screen overflow-hidden'>
      <header className='text-[#6b21a8] shadow-lg mb-8'>
        <ul className='flex flex-wrap gap-8 pt-4 px-8 text-xl'>
          <li onClick={() => {setCurrentTab("service")}} className='cursor-pointer'>
            <p>{selectedYpCandidate.service}</p>
      ...
      <div className='overflow-y-scroll h-full pb-24 scrollbar-thin'>
        {(currentTab === "service") && (
          <>
            <div className="px-4 md:px-0 md:pr-4 font-poppins pb-12">
              <header className='flex flex-col pb-2 sticky top-0 z-20 bg-white'>
                ...
                <div className='flex gap-[12px] self-end'>
                    <button onClick={goBack} className='rounded-[8px] flex items-center p-[8px] gap-[10px] border border-[#622c98] text-[#622c98]'>
                      <p>Cancel</p>
                      ...
                      <div className="w-full flex gap-4 relative">
                        ...
                        <div className="group">
                          ...
                          <div className="flex flex-col absolute left-0 top-[20px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000">
                            <div className="w-0 h-0 border-t-8 border-t-transparent border-l-8 border-l-[#6b21a8] border-r-8 border-r-transparent"></div>
          ...
          <>
            ...
            <div className='w-full overflow-x-hidden'>
              <div className='flex justify-end pr-2'>
                <div onClick={toRiskAssessment} className="flex gap-4 items-center border border-[#6b21a8] rounded-[5px] p-1 text-[#6b21a8] w-fit my-1 cursor-pointer">
                  <p>Add new</p>
              ...
              <p className='bg-[#6b21a8] text-white p-2'>Risk Assesment</p>
                ...
                <table className='border-separate border-spacing-2 md:border-spacing-1 w-full mt-4 border rounded-[5px] '>
                  <thead className='bg-gray-200'>
                    <tr className='text-center'>
                      ...
                      <th className='px-3 py-2'>Assessment date</th>
                  ...
                  {riskAssementTodisplay.length > 0 ? (
                  riskAssementTodisplay.map(riskAssementTodisplay => (
                    <tr key={riskAssementTodisplay.id} className="px-3 whitespace-nowrap text-center">
                      <td>{riskAssementTodisplay.id}</td>
                      <td>{riskAssementTodisplay.name}</td>
                      <td>{riskAssementTodisplay.assessmentDate}</td>
                      <td>{riskAssementTodisplay.createdBy}</td>
                      <td>{riskAssementTodisplay.lastModified}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
            ...
            <div className='w-full overflow-x-hidden'>
              <div className='flex justify-end pr-2'>
                <div onClick={toCaseNote} className="flex gap-4 items-center border border-[#6b21a8] rounded-[5px] p-1 text-[#6b21a8] w-fit my-1 cursor-pointer">
                  <p>Add new</p>
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H5v-2h6V5h2v6h6v2h-6v6h-2z"/></svg>
                </div>
              </div>
              ...
              <div className="lg:overflow-x-hidden overflow-x-scroll w-full">
                <table className='border-separate border-spacing-2 md:border-spacing-1 w-full mt-4 border rounded-[5px] '>
                  ...
                  <tbody className='font-medium'>
                  {caseNoteTodisplay.length > 0 ? (
                  caseNoteTodisplay.map(caseNoteTodisplay => (
                    <tr key={caseNoteTodisplay.id} className="px-3 whitespace-nowrap text-center">
                      <td>{caseNoteTodisplay.id}</td>
                      <td>{caseNoteTodisplay.startDate}</td>
                      <td>{caseNoteTodisplay.time}</td>
                      <td>{caseNoteTodisplay.endDate}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
              ...
              {selectedYpCandidate.casenote.length > 5 && (
                <button className='block mx-auto text-center p-2 text-[#6b21a8] text-lg animate-pulse my-2' onClick={() => setShowAllCaseNote(!showAllCaseNote)}>
                  {showAllCaseNote ? 'View Less' : 'View All'}
                </button>
              )}
            </div>
            ...
            <div className='w-full overflow-x-hidden'>
              <div className='flex justify-end pr-2'>
                <div onClick={toComplaints} className="flex gap-4 items-center border border-[#6b21a8] rounded-[5px] p-1 text-[#6b21a8] w-fit my-1 cursor-pointer">
                  <p>Add new</p>
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H5v-2h6V5h2v6h6v2h-6v6h-2z"/></svg>
                </div>
              </div>
              ...
              <div className="lg:overflow-x-hidden overflow-x-scroll w-full">
                <table className='border-separate border-spacing-2 md:border-spacing-1 w-full mt-4 border rounded-[5px] '>
                  ...
                  complaintsTodisplay.map(complaints => (
                    <tr key={complaints.serviceName} className="px-3 whitespace-nowrap text-center">
                      <td>{complaints.serviceName}</td>
                      <td>{complaints.dateOfComplaints}</td>
                      <td>{complaints.from}</td>
              ...
              {selectedYpCandidate.complaints.length > 5 && (
                <button className='block mx-auto text-center p-2 text-[#6b21a8] text-lg animate-pulse my-2' onClick={() => setShowAllComplaints(!showAllComplaints)}>
